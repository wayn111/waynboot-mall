...
```
waynboot-admin-api       ← 后台管理 HTTP 入口（端口 81），保留 Spring Security + JWT
waynboot-mobile-api      ← H5 商城 HTTP 入口（端口 82），使用 Sa-Token 做移动端鉴权
...
`waynboot-mobile-api` 已从原来的 Spring Security + JWT 迁移到 Sa-Token。后台 `waynboot-admin-api` 不受影响，仍使用 Spring Security + JWT，因为后台接口大量依赖 `@PreAuthorize` 权限表达式。

迁移后的目标很明确：

- 移动端入口层减少 Spring Security 过滤链和 JWT 解析代码。
- 登录态交给 Sa-Token 管理，用户快照放在 Sa-Token Session 中。
- 保持前端调用契约稳定，登录接口仍返回字符串 token，请求头仍使用 `Authorization`。
- 移动端和后台鉴权隔离，避免公共模块继续把 Spring Security 传递给移动端。

### 3.1 请求头格式

移动端接口读取 `Authorization` 请求头。当前兼容两种写法：

```http
Authorization: e12111b6-2188-4c6c-9ce4-aefd1ded1917
Authorization: Bearer e12111b6-2188-4c6c-9ce4-aefd1ded1917
```

兼容逻辑在 `MobileAuthorizationHeaderFilter`。这个过滤器运行在 Sa-Token 拦截器之前，如果发现 `Bearer ` 前缀，就把请求头裁剪为裸 token；如果本来就是裸 token，则原样放行。

注意：`MobileSaTokenConfiguration` 没有启用 Sa-Token 的 `tokenPrefix` 强制校验。如果打开该配置，Sa-Token 会要求请求头必须是 `Bearer token`，裸 token 会直接被判断为未登录。

### 3.2 关键代码入口

| 类 | 职责 |
|---|---|
| `MobileSaTokenConfiguration` | 配置 Sa-Token token 名称、有效期、读取位置、白名单和跨域 |
| `MobileAuthorizationHeaderFilter` | 兼容 `Authorization: token` 和 `Authorization: Bearer token` |
| `MobileSaTokenExceptionHandler` | 统一处理移动端未登录和 Sa-Token 鉴权异常 |
| `LoginService` | 校验用户、执行 `StpUtil.login()`、写入登录用户快照 |
| `MobileSecurityUtils` | 屏蔽 Sa-Token 细节，统一读取当前用户、刷新用户快照和处理密码 |
| `LoginUserDetail` | 移动端登录用户快照，保存会员信息、token、登录时间和过期时间 |
...
`LoginService` 返回的是 Sa-Token 原始 token 字符串。前端保存后，后续请求放到 `Authorization` 请求头即可。

### 3.4 获取当前用户

移动端业务代码不要直接调用 Sa-Token API，也不要重新解析请求头。统一使用：

```java
MobileSecurityUtils.getUserId();
MobileSecurityUtils.getLoginUser();
MobileSecurityUtils.refreshLoginUser(loginUser);
```

原因是 `MobileSecurityUtils` 统一封装了 Sa-Token Session Key 和异常转换。用户资料、头像、密码修改成功后，需要调用 `refreshLoginUser()` 把最新会员快照写回 Session，避免接口继续读到旧数据。

### 3.5 匿名接口白名单

匿名访问规则集中在 `MobileSaTokenConfiguration.EXCLUDE_PATHS`，包括登录、验证码、首页、分类、商品详情、搜索、支付回调等接口。新增移动端接口时要先判断是否必须匿名访问：

- 需要登录：不要加白名单，让 Sa-Token 拦截器统一校验。
- 不需要登录：补充到 `EXCLUDE_PATHS`，同时确认接口不会泄露用户私有数据。

### 3.6 常见未登录排查

如果登录后访问接口仍提示未登录，按顺序检查：

```text
1. 登录接口返回的 data 是否就是 token 字符串
2. 前端是否把 token 放到了 Authorization 请求头
3. 请求头是否被网关、代理或浏览器跨域配置过滤
4. Redis 是否正常，Sa-Token Session 是否能写入
5. 请求路径是否命中移动端服务，而不是后台服务
6. 是否误删或绕过了 MobileAuthorizationHeaderFilter
```

最常见问题是只传了裸 token，而 Sa-Token 被配置成强制 `Bearer` 前缀。当前代码已经通过 `MobileAuthorizationHeaderFilter` 和 `setTokenPrefix(null)` 规避这个问题。
...
**Q：为什么移动端用 Sa-Token，后台还用 Spring Security？**
A：移动端接口以登录态校验为主，Sa-Token 足够轻量，能减少 JWT 过滤器和 Security 上下文适配代码。后台接口已有大量 `@PreAuthorize` 和权限表达式，继续保留 Spring Security 风险更低。两套鉴权只在入口模块内生效，不要把 Spring Security 依赖重新放回 `common` 或 `util`。

**Q：为什么库存要用 Redis Lua 预占，而不是直接操作 MySQL？**  
A：高并发下单时，MySQL 行锁会成为瓶颈。Redis Lua 脚本原子执行，先在内存里做并发门控，只有预占成功的请求才进入 MySQL，大幅减少数据库压力。MySQL 的条件更新是最终一致性保障，防止 Redis 和 MySQL 不一致时超卖。展开看「八、8.2 防多卖 / 防少卖的真正机制」。
